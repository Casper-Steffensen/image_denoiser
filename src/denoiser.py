...
def closest_multiple(num_lines, p):
    if num_lines % p:  # := if(0) == if(False)
        return num_lines - (num_lines % p)  # num_lines?
    else:
        return num_lines
...
img = Image.open(os.getcwd()+"/inputs/test_pic.jpeg").convert("L")
img_array = np.array(img)

print("got shape: ", img_array.shape)

img_array_th_fltrd = filter_threshold(img_array)

for _ in range(2):  # why 2?
...
im.save(os.getcwd()+"/ouputs/denoised_test.jpeg")
